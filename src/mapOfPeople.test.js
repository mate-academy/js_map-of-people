...
test('Single person', () => {
  ...
  expect([...map.keys()]).toEqual([23]);
  expect([...map.values()]).toEqual([0]);
});
...
test('Sample test', () => {
  ...
  expect([...map.keys()]).toEqual([3, 1]);
  expect([...map.values()]).toEqual([1, 5]);
});

test('5 people, no one leaving', () => {
  const map = mapOfPeople([3, 2, 5, 4, 1]);

  expect([...map.keys()]).toEqual([3, 2, 5, 4, 1]);
  expect([...map.values()]).toEqual([0, 1, 2, 3, 4]);
});
...
test('10 people, 20 events', () => {
  ...
  expect([...map.keys()]).toEqual([3, 7, 0, 8]);
  expect([...map.values()]).toEqual([6, 14, 17, 19]);
});
...
test('20 people, 100 events', () => {
  ...
  expect([...map.keys()]).toEqual([7, 14, 11, 18, 0, 12, 17, 9, 16, 5]);
  expect([...map.values()]).toEqual([53, 62, 77, 84, 90, 91, 92, 93, 96, 98]);
});
