...
function mapOfPeople(people) {
  const roomWithPeple = new Map();
  people.forEach((person, time) => {
    if (!roomWithPeple.has(person)) {
      roomWithPeple.set(person, time);
    } else {
      roomWithPeple.delete(person);
    }
  });
  return roomWithPeple;
}
