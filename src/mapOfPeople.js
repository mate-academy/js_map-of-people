...
function mapOfPeople(people) {
  const peopleInMap = new Map();
  const isPersonInRoom = (user, time) => {
    if (peopleInMap.has(user, time)) {
      peopleInMap.delete(user);
    } else {
      peopleInMap.set(user, time);
    }
  };

  people.forEach(isPersonInRoom);
